...
/* 커스텀 오버레이 */
.wrap {position: absolute;left: 0;bottom: 57px;width: 388px;height: 367px;margin-left: -194px;text-align: left;overflow: hidden;font-size: 12px;font-family: 'Malgun Gothic', dotum, '돋움', sans-serif;line-height: 1.5;}
.wrap * {padding: 0;margin: 0;}
.wrap .info {width: 386px;height: 355px;border-radius: 5px;border-bottom: 2px solid #ccc;border-right: 1px solid #ccc;overflow: hidden;background: #fff;}
...
.info .desc {overflow: auto; position: relative; margin: 13px 0 0 110px; height: 300px;}
...
<div class="row">
	<h4>
		<strong>#</strong> 검색 키워드 : <strong>${requestScope.partner} ${requestScope.subname} ${requestScope.address} ${requestScope.hosname}</strong><br/>
		<strong>#</strong> 검색 결과 &nbsp;&nbsp;: 총 <strong>${requestScope.size}개</strong>의 병원이 검색되었습니다. 
	</h4>
</div>
...
for(var i = 0; i < datas.length; i++) {
    ...
    markers[i] = new daum.maps.Marker({
              map: map,
              image: markerImage,
              position: new daum.maps.LatLng(datas[i]['lat'], datas[i]['lng']),
              zIndex: datas[i]['no']
       });
